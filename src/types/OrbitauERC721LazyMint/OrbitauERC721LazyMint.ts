...
export class OrbitauERC721LazyMint extends ethereum.SmartContract {
  static bind(address: Address): OrbitauERC721LazyMint {
    return new OrbitauERC721LazyMint("OrbitauERC721LazyMint", address);
  }
  ...
  tokenType(tokenId: BigInt): BigInt {
    let result = super.call("tokenType", "tokenType(uint256):(uint256)", [
      ethereum.Value.fromUnsignedBigInt(tokenId)
    ]);
  ...
  try_tokenType(tokenId: BigInt): ethereum.CallResult<BigInt> {
    let result = super.tryCall("tokenType", "tokenType(uint256):(uint256)", [
      ethereum.Value.fromUnsignedBigInt(tokenId)
    ]);
    if (result.reverted) {
      return new ethereum.CallResult();
